...
// old 2.3 definition: #define MUL(d,a,b)	(TEMPORARY_MULT = reg(a)*reg(b), DO_SETREG(TMP,unsigned(TEMPORARY_MULT)>>16), DO_SETREG((d),unsigned(TEMPORARY_MULT)&0xffff), flags[flag_s] = TEMPORARY_MULT<0, flags[flag_z] = (TEMPORARY_MULT == 0), flags[flag_v] = TEMPORARY_MULT != (int) reg(d));
...
#define CHECK_FLAGS4(u4) ((((u4)<0)|(((unsigned) u4)>0x0f))?((void)Herror("Illegal flag specification"),((u4)&0x0f)):(u4))
...
#define CALL(r_new_FP, l_for_PC)					\
  ...
  /* Rb=PC+1 */	DO_SETREG(r_RA, __LINE__ + 1);				\
		if (    reg(r_new_FP)+TYPICAL_FRAME_MAX_SIZE > first_space_for_heap || \
			reg(SP)                              > first_space_for_heap) { \
